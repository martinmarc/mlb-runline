...
begin_date = (datetime.today() - timedelta(days = 7)).strftime("%Y-%m-%d")
...
team_1_away_wins["spread"] = team_1_away_wins["away_score"] - team_1_away_wins["home_score"]

team_1_home_wins = Featured_Merged_DataFrame[Featured_Merged_DataFrame["home_name"] == Featured_Merged_DataFrame["team_1"]].copy()
team_1_home_wins["spread"] = team_1_home_wins["home_score"] - team_1_home_wins["away_score"]
...
existing_data = pd.read_sql("SELECT * FROM baseball_spread", con = engine)
existing_data["unique_indentifier"] = existing_data["game_datetime"].astype(str) + existing_data["team_1"].astype(str) + existing_data["team_2"].astype(str) + existing_data["venue_name"].astype(str)

Featured_Spread_DataFrame["unique_indentifier"] = Featured_Spread_DataFrame.index.strftime("%Y-%m-%d %H:%M:%S") +  Featured_Spread_DataFrame["team_1"].astype(str) + Featured_Spread_DataFrame["team_2"].astype(str) + Featured_Spread_DataFrame["venue_name"].astype(str) 

new_data = Featured_Spread_DataFrame[~Featured_Spread_DataFrame["unique_indentifier"].isin(existing_data["unique_indentifier"])]
new_data = new_data.drop("unique_indentifier", axis = 1)

new_data.to_sql("baseball_spread", con = engine, if_exists = "append")